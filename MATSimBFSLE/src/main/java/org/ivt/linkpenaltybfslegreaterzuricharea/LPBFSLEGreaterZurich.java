...
*
* LPBFSLE stands for Link Penalty Breadth First Search on Link Elimination.
* <p>
* the original breadth first search on link elimination algorithm is due to Michael Balmer. This is an adapted version (by Matteo Felder)
* that uses a cost function which may include an additional link penalty, and is specific to the greater Zurich area example
* in the sense that the attributes used in the cost function are specific to this use case.
*/
...
public class LPBFSLEGreaterZurich implements ChoiceSetGenerator {
	
	//////////////////////////////////////////////////////////////////////
	// member variables
	//////////////////////////////////////////////////////////////////////
	
	private final static Logger log = Logger.getLogger(LPBFSLEGreaterZurich.class);
	 ...
	 */
	private final BrouterCostFunctionGreaterZurich cost_function;
	...
	/**
	 * Map recording the links that have already been used.
	 */
	private final Map<String,Double> used_links = new HashMap<>();
	...
	public LPBFSLEGreaterZurich(Network network) {
		if (network == null) { throw new RuntimeException("Network must exist."); }
		
		this.network = network;
		cost_function = new BrouterCostFunctionGreaterZurich();
		this.initializeUsedLinks();
		cost_function.setUsedLinks(used_links);
		
		globalConfigGroup = new GlobalConfigGroup();
		
		router = new AStarLandmarksFactory(globalConfigGroup).createPathCalculator(network, cost_function, cost_function);
	...
	/**
	 * Records the number of times a link has been used.
	 * @param link
	 */
	private void addCostToUsedLink(Link link) {
		Double number_of_times = used_links.get(link.getId().toString());
		number_of_times += 1.0;
		used_links.put(link.getId().toString(), number_of_times);
	}
	
	/**
	 * sets up dictionary: at the start the choice set is empty and each link has been used zero times.
	 */
	private final void initializeUsedLinks() {
		long timeBeforeInitUsedLinks = System.currentTimeMillis();
		
		for (Link l : network.getLinks().values()) {
			used_links.put(l.getId().toString(), 0.0);
		}
		long calcTimeInitUsedLinks = System.currentTimeMillis()-timeBeforeInitUsedLinks;
		log.warn("It took us " + calcTimeInitUsedLinks + " ms to init used links.");
	}
