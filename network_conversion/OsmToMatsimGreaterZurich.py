...
##df = pd.read_csv('/Users/matteofelder/Documents/IVT/bike_zurich/stadt_zh_network/chosen_routes/chosen_routes_final.tsv', sep='\t')
...
#
# df_2020_bike = pd.read_csv('/Users/matteofelder/Documents/IVT/bike_zurich/2020/bike/chosen_routes/chosen_routes_bike_2020.tsv', sep='\t')
# df_2020_ebike = pd.read_csv('/Users/matteofelder/Documents/IVT/bike_zurich/2020/ebike/chosen_routes/chosen_routes_ebike_2020.tsv', sep='\t')
...
df_bike = pd.read_csv('/Users/matteofelder/Documents/IVT/bike_zurich/greater_zurich/bike/chosen_routes/chosen_routes_bike.tsv', sep='\t')
df_ebike = pd.read_csv('/Users/matteofelder/Documents/IVT/bike_zurich/greater_zurich/ebike/chosen_routes/chosen_routes_ebike.tsv', sep='\t')

chosen_links_list = []

# df_2019_bike = df_2019_bike.replace(np.nan, 0)
# for index, row in df_2019_bike.iterrows():
#     for link in row[3:-1]:
#         if link != 0:
...
#
#
# df_2020_bike = df_2020_bike.replace(np.nan, 0)
# for index, row in df_2020_bike.iterrows():
#     for link in row[3:-1]:
#         if link != 0:
#             chosen_links_list.append(link)
#
# df_2020_ebike = df_2020_ebike.replace(np.nan, 0)
# for index, row in df_2020_ebike.iterrows():
#     for link in row[3:-1]:
#         if link != 0:
#             chosen_links_list.append(link)
#
# df_2021_bike = df_2021_bike.replace(np.nan, 0)
# for index, row in df_2021_bike.iterrows():
#     for link in row[3:-1]:
#         if link != 0:
#             chosen_links_list.append(link)
#
# df_2021_ebike = df_2021_ebike.replace(np.nan, 0)
# for index, row in df_2021_ebike.iterrows():
#     for link in row[3:-1]:
#         if link != 0:
#             chosen_links_list.append(link)

df_bike = df_bike.replace(np.nan, 0)
for index, row in df_bike.iterrows():
    for link in row[3:-1]:
        if link != 0:
            chosen_links_list.append(link)

df_ebike = df_ebike.replace(np.nan, 0)
for index, row in df_ebike.iterrows():
    for link in row[3:-1]:
        if link != 0:
            chosen_links_list.append(link)

tree = ET.parse('/Users/matteofelder/Documents/IVT/OsmToMatsim/stadt_zhrect_final_7_AOT.xml')
root = tree.getroot()
...
for elem in root.findall('node'):
    ...
    node = ET.SubElement(nodes, "node", id=str(W('id')), x=str(x), y=str(y))
    list = []
    ...
    node_attribute_ts_osm = ET.SubElement(node_attributes, "attribute", name="ts_osm")
    node_attribute_ts_osm.set("class", "java.lang.String")
    node_attribute_ts_osm.text = list[2][1]
    node_attribute_ts_osm_2 = ET.SubElement(node_attributes, "attribute", name="ts_osm_2")
    node_attribute_ts_osm_2.set("class", "java.lang.String")
    node_attribute_ts_osm_2.text = list[3][1]
...
for elem in root.findall('way'):
    ...
    for subelem in elem.findall('tag'):
        ...
        if str(subelem.get('k'))=='oneway':
            if str(subelem.get('v'))=="yes":
                ...
                list[-1][1] = 'no'
        if str(subelem.get('k'))=='lanes':
            if str(subelem.get('v'))=='nan':
                list[-1][1] = '1'
        #if str(subelem.get('v'))=="nan":
        #   list[-1][1] = "unknown"
    if list[16][1]=='False' and list[15][1]=='False' and ((list[7][1] == 'no' or list[7][1] == 'nan') and
                                                          (list[8][1] == 'no' or list[8][1] == 'nan' or list[8][1] == 'dismount')):
        continue
    if list[4][1] == 'yes':#i.e. oneway == '1', create the link as in Meister's network (exception will come later)
        ...
        link.set('freespeed', str(6.0)) # default
        ...
        if list[17][1] == 'True' or (list[16][1] =='True' and list[15][1]=='False' and list[17][1] == 'False'):
            link = ET.SubElement(links, "link", id=list[1] + "_" + list[0])
            link.set('from', list[1])
            link.set('to', list[0])
            link.set('length', list[5][1])
            link.set('freespeed', str(6.0))
            link.set("capacity", '300')  # default
        ...
        attribute_velostreif = ET.SubElement(attributes, "attribute", name="velostreifen")
        attribute_velostreif.set("class", "java.lang.String")
        if list[21][1] == 'yes': #oneway_bike (i.e. list[28][1]) tells us whether we have a velostreifen in the digitalization direction
            attribute_velostreif.text = list[20][1]
        elif list[21][1] == 'counter_yes':
            attribute_velostreif = "0.0"
        else:
            attribute_velostreif.text = list[20][1] #there might be some links for which oneway_bike is 'nan', but still velostreif = '1'
            # attribute_oneway_bike = ET.SubElement(attributes, "attribute", name="oneway_bike")
            # attribute_oneway_bike.set("class", "java.lang.String")
            # attribute_oneway_bike.text = list[28][1]
        # attribute_velomaster = ET.SubElement(attributes, "attribute", name="velomaster")
...
with open('/Users/matteofelder/Documents/IVT/OsmToMatsim/greater_zh_matsim_car_bike.xml', 'wb') as f:
